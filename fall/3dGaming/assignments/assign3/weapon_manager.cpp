...
// Use g->setVelocity
	...
	g->setPosition(pos);
	// Set velocity here
	Vector3 origin = pos, dir = direction, diff = pos-direction;
	diff.normalise();
	origin.normalise();
	
	Real angle = diff.dotProduct(origin); 
	//std::cout<<"angle: "<<angle<<", acos:"<<acos(angle)<<std::endl;
	
	g->setVelocity(direction*200);
	//g->setPosition((direction*10 +pos + mVelocity));
	g->setSpeedFactor(1);
	g->setLife(5, 5);
	g->makeAlive();
