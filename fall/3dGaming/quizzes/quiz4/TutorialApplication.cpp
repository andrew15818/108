...
BasicTutorial_00::BasicTutorial_00(void)
{
    ...
    m_TimeStep = 0.02; //originally 0.06
}
...
{
	//attaching each sphere node to a particle in our SMS
	m_MassSpringSystem->setSphereArrangementParams(this->m_NumCircles, this->m_NumSamples);
	for(int i=0; i<mNumSpheres; i++){
		bool movable = (i<8)?false:true;
		m_MassSpringSystem->addParticle(mSceneNode[i], mSmallSphereRadius ,movable);
	}
	int index = 0;
	for(int i=0; i<m_NumCircles-2; i++){
		//if( i== m_NumCircles-2){break;}
		for(int j=0; j < m_NumSamples; j++, index++){
			m_MassSpringSystem->addSpring(index, index+m_NumSamples); //the sphere directly next to it
			m_MassSpringSystem->addSpring(index, index+(2*m_NumSamples));
			std::cout<<"added spring between ("<<index<<","<<index+m_NumSamples<<
				"and ("<<index<<","<<index+(2*m_NumSamples)<<std::endl;
		}
	}
}
...
{
	//samples is how many lines of balls we have
	//numCircles is how many balls in each line
	std::cout<<"NUMSPHERES: "<<mNumSpheres<<std::endl;
    float r0 = 50.0; // radius of the first circle
    float dr = 12.0; // delta distance
    float h = 100.0; // height
	float x, y, z; //positions for our spheres
	int index = 0;
    for (int j = 0; j < numCircles; ++j) {
        float r = r0 + dr*j;
        for (int i = 0; i < numSamples; ++i, index++) {
            float a = i/(float) numSamples*3.14*2.0; //angle from the origin
            x = r * cos(a);
			z = r * sin(a);
			y=100;
            //x, y, z = ? 
...
void BasicTutorial_00::createSpace()
{
	...
	//samples are the amount of lines we have, m_NumCircles is the amount of circles per line
    m_NumSamples = 8; //amount of individual lines
    m_NumCircles = 15; //number of spheres per line
...
bool BasicTutorial_00::keyPressed( const OIS::KeyEvent &arg )
{
    ...
    if (arg.key == OIS::KC_K ) {
		std::cout<<"no robust collision"<<std::endl;
        m_MassSpringSystem->setCollisionConstraint(false);
        setBackground();
    }
    if (arg.key == OIS::KC_L ) {
		std::cout<<"with robust collision"<<std::endl;
