...
for i in range(0, c1.shape[0]):
    # separate the two coluns so we can transpose the array
    tmp = (c1[i] - m1)[np.newaxis] 
    sw1 += np.matmul(tmp.T, tmp)

for i in range(0, c2.shape[0]):
    tmp = (c2[i] - m2)[np.newaxis]
    sw2 += np.matmul(tmp.T, tmp)
sw = sw1 + sw2
assert sw.shape == (2, 2)
...
# ## 5. Project the test data by linear discriminant to get the class prediction by 
# nearest-neighbor rule and calculate the accuracy score
...
# For each point in testing set, 
# calculate nearest point in training set
for i in range(0, test_projection.shape[0]):
    min_dist = np.absolute(test_projection[i] - 
                train_projection[0])  # just placeholder min distance
    best_index = 0
...
print(f"rise : {rise} \t run: {run} \t slope {slope} 
...
# plotting classes individually on projection line
proj_line_c1 = projection_line[y_train == 1]
proj_line_c2 = projection_line[y_train == 0]

plt.plot(proj_line_c1[:, 0] + 1.5, proj_line_c1[:, 1], '.', color="red")
plt.plot(proj_line_c2[:, 0] + 1.5, proj_line_c2[:, 1], '.', color="green")

plt.plot(c1[:, 0], c1[:, 1],'+' ,color="green")
plt.plot(c2[:, 0], c2[:, 1],'+' ,color="red")
plt.title("slope: " + str(slope) + " b: " + str(y_intercept))
plt.show()
