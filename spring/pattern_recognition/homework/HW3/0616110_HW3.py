...
class Node():
    def __init__(self,):
        ...
        # how many nodes to the left and right, used for feature importance
        self.less_than_size = 0
        self.greater_than_size = 0
        self.data_size = 0
        # threshold on which we split the data
        self.threshold = 0

        # name of feature for this node
        self.feature = ""
        self.purity = 1
...
class DecisionTree():
    def __init__(self, criterion='gini', max_depth=None, features_list=feature_names):
        # either gini or entropy to get the purity
        self.criterion = criterion

        # how many layers or binary tree may go down
        self.max_depth = max_depth

        # since we can use it with a custom set of feature for random forest
        # specify which set of features we want to use
        self.features_list = features_list
        self.root = Node()
        self.feature_importance = {}
        self.root = self.tree(x_train, self.root)

    # for plotting the importance
    ...
    def tree(self, data, parent):
        ...
        curr_node.less_than_size = best_less_than.shape[0]
        curr_node.greater_than_size = best_greater_than.shape[0]
        curr_node.data_size = data.shape[0]
        ...
        imp = ((data.shape[0]  / x_train.shape[0]) 
                * (curr_node.purity - curr_node.right.greater_than_size 
                / data.shape[0] 
                * curr_node.right.purity 
                - curr_node.left.data_size / data.shape[0] 
                * curr_node.left.purity))

        if curr_node.feature in self.feature_importance:
            self.feature_importance[curr_node.feature] += imp
        else:
            self.feature_importance[curr_node.feature] = imp
        return curr_node
...
class RandomForest():
    ...
    def gen_features_list(self):
        ...
        while i < self.max_features:
            ...
            index = randrange(len(feature_names_orig)-1)
...
clf_all_features.build_forest(x_test)
...
clf_random_features.build_forest(x_test)
